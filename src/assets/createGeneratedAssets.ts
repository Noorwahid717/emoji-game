...
const createEmojiTextures = (scene: Phaser.Scene): void => {
  EMOJI_CHARACTERS.forEach((emoji, index) => {
    ...
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(emoji, CARD_SIZE / 2, CARD_SIZE / 2 + 2);

    const key = `emoji-${index}`;
    scene.textures.addCanvas(key, ctx.canvas);
  });
};
