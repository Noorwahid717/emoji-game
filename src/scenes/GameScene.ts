...
class GameScene extends Phaser.Scene {
  ...
  public create(): void {
    ...
    this.hud = new Hud(this, { highScore });
  ...
  private createCard(card: CardDefinition, index: number): CardSprite {
    ...
    sprite.faceTexture = card.textureKey;
  ...
  private evaluateSelection(): void {
    ...
    this.resolving = true;
    if (first.faceTexture === second.faceTexture) {
  ...
  private flipCard(card: CardSprite, faceUp: boolean): void {
    const targetTexture = faceUp ? card.faceTexture : 'card-back';
    this.tweens.add({
      targets: card,
      scaleX: 0,
      duration: 100,
